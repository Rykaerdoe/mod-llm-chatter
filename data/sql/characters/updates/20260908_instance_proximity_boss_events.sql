-- Allow safe pre-aggro boss approach and directed proximity events.

SET @has_proximity_boss_events = (
  SELECT COUNT(*) FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'llm_chatter_events'
    AND COLUMN_NAME = 'event_type'
    AND COLUMN_TYPE LIKE '%proximity_boss_approach%'
    AND COLUMN_TYPE LIKE '%proximity_boss_player_say%'
);

SET @sql = IF(
  @has_proximity_boss_events = 0,
  CONCAT(
    ...
    "'proximity_boss_approach','proximity_boss_player_say',",
