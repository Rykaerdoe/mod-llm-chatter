-- Add directed NPC emote events and distinguish consumed drops from speech.

SET @has_proximity_player_emote = (
  SELECT COUNT(*) FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'llm_chatter_events'
    AND COLUMN_NAME = 'event_type'
    AND COLUMN_TYPE LIKE '%proximity_player_emote%'
);

SET @sql = IF(
  @has_proximity_player_emote = 0,
  CONCAT(
    ...
    "'guild_player_message','guild_login_greeting') NOT NULL"
  ),
  'SELECT 1'
);
...
SET @has_drop_reason = (
  ...
  WHERE TABLE_SCHEMA = DATABASE()
    ...
    AND COLUMN_NAME = 'drop_reason'
);

SET @sql = IF(
  @has_drop_reason = 0,
  CONCAT(
    "ALTER TABLE `llm_chatter_messages` ADD COLUMN ",
    "`drop_reason` VARCHAR(64) DEFAULT NULL AFTER `delivered`"
  ),
  'SELECT 1'
);
PREPARE stmt FROM @sql;
...
SET @has_addressee_player_guid = (
  SELECT COUNT(*) FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'llm_chatter_messages'
    AND COLUMN_NAME = 'addressee_player_guid'
);

SET @sql = IF(
  @has_addressee_player_guid = 0,
  CONCAT(
    "ALTER TABLE `llm_chatter_messages` ADD COLUMN ",
...
SET @has_addressee_bot_guid = (
  SELECT COUNT(*) FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'llm_chatter_messages'
    AND COLUMN_NAME = 'addressee_bot_guid'
);

SET @sql = IF(
  @has_addressee_bot_guid = 0,
  CONCAT(
    "ALTER TABLE `llm_chatter_messages` ADD COLUMN ",
    "`addressee_bot_guid` INT UNSIGNED DEFAULT NULL ",
    "AFTER `addressee_player_guid`"
  ),
  'SELECT 1'
);
PREPARE stmt FROM @sql;
...
SET @has_addressee_npc_spawn_id = (
  SELECT COUNT(*) FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'llm_chatter_messages'
    AND COLUMN_NAME = 'addressee_npc_spawn_id'
);

SET @sql = IF(
  @has_addressee_npc_spawn_id = 0,
  CONCAT(
    "ALTER TABLE `llm_chatter_messages` ADD COLUMN ",
    "`addressee_npc_spawn_id` INT UNSIGNED DEFAULT NULL ",
    "AFTER `addressee_bot_guid`"
  ),
  'SELECT 1'
);
PREPARE stmt FROM @sql;
